...
public:
  ...
  PGL(ScenePtr)
  sceneInterpretation(AxialTree &workstring);
  ...
  PGL(ScenePtr)
  getLastComputedScene() { return __lastcomputedscene; }
  ...
  public:
    ...
    virtual bool shouldStop(AxialTree::const_iterator match) const
    {
      if (alwaysStop)
        ...
        return lstringHasBreakPointAt(match);
    }
    ...
    inline void removeCodeBreakPointAt(uint32_t breakpoint)
    {
      ...
      if (it == linebreaks.end())
        LsysError("Cannot remove code break point.");
      else
        linebreaks.erase(it);
    }
    ...
    inline void removeLstringBreakPointAt(AxialTree::const_iterator breakpoint)
    {
      ...
      if (it == lstringbreaks.end())
        LsysError("Cannot remove lstring break point.");
      else
        lstringbreaks.erase(it);
    }
...
protected:
  ...
  PGL(ScenePtr)
  __apply_post_process(AxialTree &workstring, bool endeach = true);
