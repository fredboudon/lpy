...
Lsystem::RuleGroup::RuleGroup() : __prodhasquery(false),
                                  __dechasquery(false),
                                  __inthasquery(false)
{
}
...
Lsystem::Lsystem() : __max_derivation(1),
                     ...
                     Lsystem::Lsystem(const std::string &filename) : __max_derivation(1),
                                                                     __decomposition_max_depth(1),
                                                                     __interpretation_max_depth(1),
                                                                     __context(),
                                                                     __newrules(false)
...
Lsystem::Lsystem(const std::string &filename,
                 const boost::python::dict &parameters) : __max_derivation(1),
                                                          __decomposition_max_depth(1),
                                                          __interpretation_max_depth(1),
                                                          __context(),
                                                          __newrules(false)
...
Lsystem::Lsystem(const Lsystem &lsys) : __rules(lsys.__rules),
...
Lsystem::str() const
{
  ...
  if (!__context.__modules.empty())
  {
    ...
    {
      if (it != __context.__modules.begin())
        s << ',';
      s << (*it)->name;
    }
    s << '\n';
...
void Lsystem::__importPyFunctions()
{
  ContextMaintainer m(&__context);
  RuleSet::iterator i;
...
void Lsystem::read(const std::string &filename,
...
void Lsystem::addProductionRule(const std::string &code, size_t group, const ConsiderFilterPtr filter)
{
  // ACQUIRE_RESSOURCE
  ContextMaintainer m(&__context);
...
void Lsystem::addDecompositionRule(const std::string &code, size_t group, const ConsiderFilterPtr filter)
{
  // ACQUIRE_RESSOURCE
  ContextMaintainer m(&__context);
...
void Lsystem::addInterpretationRule(const std::string &code, size_t group, const ConsiderFilterPtr filter)
{
  // ACQUIRE_RESSOURCE
  ContextMaintainer m(&__context);
...
void Lsystem::addRule(const LsysRule &rule, int type, size_t groupid)
{
  switch (type)
  {
  case 1:
    ...
    if (rule.hasQuery())
      __group(groupid).__dechasquery = true;
    break;
  case 2:
    ...
    if (rule.hasQuery())
      __group(groupid).__inthasquery = true;
    break;
  default:
    __group(groupid).production.push_back(rule);
    if (rule.hasQuery())
      __group(groupid).__prodhasquery = true;
    break;
  }
  __newrules = true;
}
...
void Lsystem::setAxiom(const AxialTree &axiom)
{
  ACQUIRE_RESSOURCE
  __axiom = axiom;
  RELEASE_RESSOURCE
...
RulePtrMap Lsystem::__getRules(eRuleType type, size_t groupid, eDirection direction, bool *hasQuery)
{
  ...
  if (groupid >= nbgroups)
  {
    if (nbgroups == 0)
      ...
      return __getRules(type, 0, direction, hasQuery);
  }
  RulePtrSet result;
                               ...
                               Debugger &debugger)
{
  ...
  if (query)
    __turtle_interpretation(workingstring, __context.envturtle);
        ...
        if ((*_it2)->reverse_match(workingstring, _it, targetstring, _it3, args, &itermap))
        {
          ...
          if (match)
          {
            if (debugger.shouldStop(_it3 == _end ? _beg : _it3 + 1, _it + 1, *_it2))
              debugger.total_match(_it3 == _end ? _beg : _it3 + 1, _it + 1, targetstring, prodlength, *_it2, args);
            _it = _it3;
            break;
          }
                   ...
                   const RulePtrMap &ruleset)
{
  ...
  if (workingstring.size() >= beg + size)
    itend += size;
  else
                                      ...
                                      size_t size)
{
  ...
  //std::cout << "Arrived at the end of function call" << std::endl;
                        ...
                        eDirection direction)
{
  ...
  if (nbsymbolperthread * maxnbthread < lstringsize)
    ++nbsymbolperthread;
                            ...
                            StringMatching &matching)
{
  ...
  targetstring.reserve(workingstring.size());
  if (query)
    LPY::turtle_interpretation(workingstring, __context.turtle);
  AxialTree::const_iterator _it = workingstring.begin();
  AxialTree::const_iterator _it3 = _it;
  AxialTree::const_iterator _endit = workingstring.end();
  AxialTree::IteratorMap itermap;
...
void Lsystem::__recursiveInterpretation(AxialTree &workingstring,
                                        const RulePtrMap &ruleset,
                                        Turtle &t,
                                        boost::python::object &pyturtle,
                                        size_t maxdepth)
...
void Lsystem::__recursiveStepInterpretation(AxialTree &workingstring,
                                            const RulePtrMap &ruleset,
                                            PglTurtle &t,
                                            boost::python::object &pyturtle,
                                            size_t maxdepth)
      ...
      for (; (matching || no_match_no_return) && i < nb_iter; ++i)
      {
        ...
        if (!production.empty())
        {
          if (!hasDebugger())
            ...
            workstring = __debugStep(workstring, production, previouslyinterpreted ? false : productionHasQuery, matching, dir, *__debugger);
...
void Lsystem::__apply_pre_process(AxialTree &workstring, bool starteach)
{
  // Call endeach function
  object result;
...
void Lsystem::plot(AxialTree &workstring, bool checkLastComputedScene)
{
  ACQUIRE_RESSOURCE
...
void Lsystem::turtle_interpretation(AxialTree &workstring, PGL::Turtle &t)
{
  ACQUIRE_RESSOURCE
...
void Lsystem::__turtle_interpretation(AxialTree &wstring, PGL::Turtle &t, boost::python::object pyturtle)
{
  if (pyturtle == boost::python::object())
    pyturtle = boost::python::object(boost::cref(t));
  if (wstring.empty())
    return;
  bool homHasQuery = false;
  RulePtrMap interpretation = __getRules(eInterpretation, __currentGroup, eForward, &homHasQuery);
...
void Lsystem::__plot(AxialTree &workstring, bool checkLastComputedScene)
{
  ScenePtr result;
...
void Lsystem::record(const std::string &prefix,
