...
{
	if (mod->isPredefined())
		return ((PredefinedModuleClass *)mod)->getCategory();
	else
		return PredefinedModuleClass::eUserDefined;
}
...
void export_ModuleClass()
{
	...
	class_<ModuleClass, ModuleClassPtr, boost::noncopyable>("ModuleClass", no_init)
		.enable_pickling()
	...
	class_<ModuleClassTable, boost::noncopyable>("ModuleClassTable", no_init)
		// .def("__repr__",&mc_repr)
