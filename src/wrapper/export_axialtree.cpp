...
boost::python::object iter_to_int(AxialTree *tree, AxialTree::const_iterator pos)
{
	if (pos == tree->end())
		...
		return boost::python::object(tree->pos(pos));
}
...
void export_AxialTree()
{
	...
	class_<AxialTree>("AxialTree", init<>("AxialTree()"))
		.enable_pickling()
 ...
 */
	...
	class_<PyAxialTreeIterator>("AxialTreeIterator", init<AxialTree>("AxialTreeIterator(AxialTree)"))
