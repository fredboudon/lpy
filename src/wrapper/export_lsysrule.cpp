...
object match(LsysRule *rule, const AxialTree &tree, int pos, const AxialTree &dest)
{
  ...
  ArgList args;
  if (!rule->match(tree, beg, dest, endpos, args))
    return object(false);
...
object reverse_match(LsysRule *rule, const AxialTree &tree, int pos, AxialTree &dest)
{
  ...
  AxialTree::const_iterator endpos;
  ArgList args;
  if (!rule->reverse_match(tree, beg, dest, endpos, args))
    return object(false);
  return boost::python::make_tuple(tree.pos(endpos), args);
}
...
void export_LsysRule()
{
  ...
  class_<ArgList>("ArgList", init<optional<size_t, bp::object>>("ArgList(size,obj)"))
      .def(vector_indexing_suite<ArgList, true>())
      .def("__repr__", &py_rpr_ag)
      .def("__str__", &py_str_ag);
#endif

  class_<LsysRule>("LsysRule", init<optional<size_t, size_t, char>>("LsysRule(id,group,prefix)"))
      .enable_pickling()
